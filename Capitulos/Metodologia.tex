...
Las transcripciones automáticas de los vídeos suelen presentar irregularidades gramaticales. Por ello, en primer lugar, el texto fue estandarizado transformando los valores nulos y forzando la compatibilidad de \textit{string} para evitar fallos de ejecución. A continuación, tal y como se describió en el marco teórico, se aplicó un proceso de limpieza mediante expresiones irregulares (\textit{regex}). Este proceso convirtió el texto a minúsculas y eliminó sistemáticamente enlaces webs (\textit{URLs}), menciones directas a usuarios, etiquetas (\textit{hashtags}), emoticonos (\textit{emojis}) y prefijos estructurales de la transcripción (como \texttt{``AUDIO:''} o \texttt{``VIDEO:''}).
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Comentario}: El mensaje contiene una clara cosificación y un uso despectivo hacia las mujeres (``\textit{chicks}''), pero el modelo no logró captar el sesgo implícito. Esto se debe a que la ofensa es sutil, se presenta como una falsa preferencia estética y está altamente diluida por el ruido de las transcripciones de la red social (``\textit{replying to... comment}''), un formato de metadato conversacional extraño de \textit{TikTok} que evadió los filtros de limpieza de expresiones regulares, confundiendo a la red neuronal.
    \end{itemize}
\end{itemize}
...
En segundo lugar, se identificó un problema derivado de las limitaciones en la transcripción automática de los audios y de los textos capturados mediante \textit{OCR}. Aunque el preprocesamiento eliminó elementos gráficos directos como los emojis y las \textit{URLs}, muchas de las instancias fallidas aún contenían problemas severos de transcripción: nombres de usuario aleatorios integrados en el texto, lectura fragmentada de caracteres y errores fonéticos propios de los subtítulos automáticos. Este tipo de ruido dificulta que la atención matricial de las redes neuronales aísle el verdadero mensaje sexista. 
...
En la \autoref{fig:ResultadosTransformersPerspectivismoHombreDiscard} y en la \autoref{fig:ResultadosTransformersPerspectivismoMujerDiscard}, al descartar los casos donde los anotadores no llegaron a un acuerdo, se evaluó el rendimiento de cada género ante las instancias restantes:
\begin{itemize}
    \item \textbf{Perspectiva Femenina}: El modelo instruido con estos datos muestra un comportamiento sólido, logrando un \textit{F1-score} de 0.6771. Según se observa en su respectiva matriz de confusión, destaca su capacidad para identificar la clase misógina (164 Verdaderos Positivos frente a los 124 del modelo masculino), manteniendo un \textit{Recall} de 0.68.
    \item \textbf{Perspectiva Masculina}: Presenta un rendimiento inferior bajo esta estrategia (F1: 0.6462). Como evidencia su matriz, su \textit{Recall} para la clase misógina decae a 0.51, fallando prácticamente en la mitad de las predicciones positivas (117 Falsos Negativos frente a 124 Verdaderos Positivos) cuando se trata de detectar instancias que han superado el filtro de descarte.
\end{itemize}
...
En la \autoref{fig:ResultadosTransformersPerspectivismoHombrePessimistic} y en la \autoref{fig:ResultadosTransformersPerspectivismoMujerPessimistic}, al entrenar a los modelos asumiendo la etiqueta sexista en caso de empate, los resultados revelan el mejor escenario para los datos femeninos:
\begin{itemize}
    \item \textbf{Perspectiva Femenina}: Alcanza aquí su mejor rendimiento global, con un \textit{F1-score} de 0.6860. Forzar esta ``sospecha'' algorítmica no dispara sus Falsos Positivos de forma descontrolada (mantiene una Precisión de 0.72 para la clase positiva, tal y como refleja su matriz), indicando un buen equilibrio entre sensibilidad y precisión.
    \item \textbf{Perspectiva Masculina}: Por el contrario, el modelo entrenado con datos masculinos disminuye su rendimiento bajo esta directriz (F1: 0.6453). Al obligarle a asumir la etiqueta sexista en los empates, el modelo comete una mayor cantidad de errores al clasificar instancias inofensivas, lo que reduce su \textit{Accuracy} a 0.6494.
\end{itemize}
...
En la \autoref{fig:ResultadosTransformersPerspectivismoHombreOptimistic} y en la \autoref{fig:ResultadosTransformersPerspectivismoMujerOptimistic}, donde se entrena asumiendo la etiqueta no sexista en los empates, se observa una diferencia en el comportamiento predictivo de ambas redes:
\begin{itemize}
    \item \textbf{Perspectiva Masculina}: El modelo tiende a ser mucho más conservador. Logra un \textit{Accuracy} general ligeramente superior (0.6713), pero a costa de clasificar como inofensivos muchos casos reales de agresión, reflejado en un \textit{Recall} de 0.54 (110 Falsos Negativos en su matriz).
    \item \textbf{Perspectiva Femenina}: Se observa una mayor sensibilidad estadística ante la clase minoritaria. Aunque su \textit{Accuracy} decae (0.6534), su \textit{Recall} para la clase misógina aumenta a 0.71. Esto sugiere que, ante la instrucción de descartar la agresión en caso de duda, los datos femeninos conservan características discriminativas más robustas, reduciendo los Falsos Negativos (71 frente a los 110 del modelo masculino).
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{La necesidad de movimiento para detectar la Cosificación}: El hallazgo más revelador de esta experimentación se encuentra en la etiqueta \textit{Objectification}. Los modelos de texto (\texttt{RoBERTa}: 0.5087) y de imagen estática (\texttt{ConvNeXt}: 0.5116) mostraron dificultades al intentar detectar la cosificación, obteniendo métricas cercanas a la clasificación aleatoria. Sin embargo, la arquitectura de visión temporal \texttt{TimeSFormer} logró despuntar (F1: \textbf{0.5399}), sugiriendo una mayor capacidad de generalización para esta categoría. Esto indica que la cosificación en formato vídeo no suele captarse de forma efectiva en un fotograma congelado o en un texto, sino en el movimiento, las poses, el enfoque de la cámara o acciones sugerentes a lo largo del tiempo.
