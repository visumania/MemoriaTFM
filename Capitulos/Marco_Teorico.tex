...
De manera clásica, atendiendo a la naturaleza de los datos de entrenamiento y al tipo de retroalimentación que recibe el sistema, distinguimos entre tres tipos de aprendizaje \cite{bishop2006pattern}:
\begin{itemize}
...
\begin{figure}[H]
    ...
    \caption{Flujo de procesamiento multimodal de vídeos (texto + imagen + vídeos) (Elaboración propia)}
...
\begin{figure}[H]
    ...
    \caption{Principal diferencia entre \textit{Machine} y \textit{Deep Learning} (Fuente: \cite{lecun2015deep})}
    \label{fig:MLvsDL}
\end{figure}
...
En el contexto del ajuste fino (\textit{fine-tuning}) de arquitecturas masivas, como los \textit{Transformers} o los \textit{LLMs} empleados en este trabajo, el enorme peso de los modelos obliga a utilizar tamaños de lote muy reducidos. Asimismo, dado que estas redes ya cuentan con un amplio conocimiento adquirido durante su preentrenamiento, el número de épocas necesarias suele ser considerablemente bajo (generalmente entre 3 y 10). Un entrenamiento prolongado en estas condiciones no solo sería ineficiente, sino que podría provocar lo que se conoce como \textit{catastrophic forgetting}, el cual destruiría las representaciones lingüísticas o visuales que el modelo ya había aprendido previamente. 
...
Por otro lado, para abordar la tarea de clasificación multietiqueta (donde un mismo registro puede presentar varias categorías de sexismo simultáneamente), el problema se modela habitualmente como un conjunto de decisiones binarias independientes. Aunque la \textbf{Entropía Cruzada Categórica} (\textit{Categorical Cross-Entropy} o \textit{CCE}) no es aplicable de forma directa al no tratarse de clases mutuamente excluyentes, el fundamento matemático se mantiene a través de la \textbf{Entropía Cruzada Binaria} (\textit{Binary Cross-Entropy} o \textit{BCE}). Esta variante aplica los principios de la entropía cruzada para evaluar la probabilidad de presencia o ausencia de cada etiqueta de manera independiente, permitiendo que el modelo determine si una categoría específica aplica o no a la entrada, sin que dicha decisión restrinja las probabilidades del resto de etiquetas \cite{nam2014large}.
...
\begin{figure}[H]
    ...
    \caption{Arquitectura de los modelos \textit{Transformers} (Fuente: \cite{vaswani2017attention})}
...
\begin{figure}[H]
    ...
    \caption{Ejemplo de funcionamiento del mecanismo de atención (Fuente: Elaboración propia)}
...
\begin{figure}[H]
    ...
    \caption{Entradas necesarias para el codificador de \textit{BERT} (Fuente: \cite{devlin2018bert})}
...
\begin{itemize}
    \item \textbf{Modelos Multilingües}, se han utilizado: 
    \begin{itemize}
        \item \texttt{mmBERT} (mediante el \textit{checkpoint} \texttt{jhu-clsp/mmBERT-base}) \cite{marone2025mmbert}.
        \item \texttt{xlm-roberta-base} (\texttt{FacebookAI/xlm-roberta-base}) \cite{mehta2023llm}.
        \item \texttt{TwHIN-BERT} (\texttt{Twitter/twhin-bert-base}) \cite{zhang2023twhin}.
        \item Idóneos para captar estructuras semánticas universales y transferir conocimiento entre idiomas.
    \end{itemize}
    \item \textbf{Modelos específicos para Español}, se han evaluado:
    \begin{itemize}
        \item \texttt{BETO} (\texttt{dccuchile/bert-base-spanish-wwm-uncased}) \cite{garrido2024maria}.
        \item \texttt{BERTin} (\texttt{bertin-project/bertin-roberta-base-spanish}) \cite{de2022bertin}.
        \item \texttt{RoBERTuito} (\texttt{pysentimiento/robertuito-base-uncased}) \cite{perez2022robertuito}, siendo este último optimizado para el lenguaje informal de redes sociales.
    \end{itemize} 
    \item \textbf{Modelos específicos para Inglés}, se ha experimentado con:
    \begin{itemize}
        \item \texttt{RoBERTa-Base} (\texttt{FacebookAI/roberta-base}).
        \item \texttt{BERTweet} (\texttt{vinai/bertweet-base}) \cite{nguyen2020bertweet} (preentrenado en mensajes cortos y jerga digital).
        \item \texttt{Electra} (\texttt{google/electra-base-discriminator}) \cite{clark2020electra} (que introduce un paradigma de reemplazo de tokens más eficiente computacionalmente).
    \end{itemize} 
\end{itemize}
...
En el marco experimental de este proyecto se ha apostado firmemente por el ajuste fino. Sin embargo, debido al masivo número de parámetros de los \textit{LLMs} modernos, el reentrenamiento completo resulta computacionalmente inviable. Para solucionar este obstáculo, se ha implementado la técnica \textbf{QLoRA} (\textit{Quantized Low-Rank Adaptation}) \cite{dettmers2023qlora} sobre tres modelos generativos de última generación: \texttt{Mistral 7B} (mediante el \textit{checkpoint} \texttt{mistralai/Mistral-7B-Instruct-v0.3}) \cite{jiang2023mistral}, \texttt{LLaMA 3.1} (\texttt{meta-llama/Llama-3.1-8B-In} \texttt{struct}) \cite{dubey2024llama3} y \texttt{Gemma 2} (\texttt{google/gemma-2-2b-it}) \cite{team2024gemma}. Esta metodología ha permitido cuantizar los pesos base de los modelos a una precisión de 4 bits, congelarlos, y entrenar únicamente matrices de bajo rango insertadas en las proyecciones matemáticas de atención.
...
\begin{figure}[H]
    ...
    \caption{Arquitectura de un \textit{ViT} (Fuente: \cite{dosovitskiy2020image})}
    \label{fig:ViT}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Ilustración de los mecanismos de atención de un \textit{TimeSformer} (Fuente: \cite{bertasius2021space})}
...
\begin{figure}[H]
    ...
    \caption{Representación de los \textit{Tubelets} de los \textit{ViViT} (Fuente: \cite{arnab2021vivit})}
    \label{fig:ViViT}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Arquitectura de \textit{VideoMAE} (Fuente: \cite{tong2022videomae})}
    \label{fig:videoMAE} 
\end{figure}
...
Los valores de los hiperparámetros en cada una de las estrategias se pueden apreciar en la \autoref{tab:ValoresHiperparametrosEstrategias}.
...
\begin{table}[H]
    ...
    \begin{tabular}{lccc}
        ...
        \textbf{\textit{\begin{tabular}[c]{@{}c@{}}Slow \\ Cooker\end{tabular}}} & 
        \textbf{\textit{\begin{tabular}[c]{@{}c@{}}Gradient \\ Accumulation\end{tabular}}} & 
        \textbf{\textit{\begin{tabular}[c]{@{}c@{}}Sprint \\ Final\end{tabular}}} \\
        \midrule 
            \textit{learning\_rate}  & 3e-5 & 5e-5 & 5e-5 \\
            \textit{weight\_decay}  & 0.1 & 0.01 & 0.05 \\
            \textit{num\_train\_epochs}  & 8 & 7 & 6 \\
            \textit{warmup\_ratio}  & 0.1 & 0 & 0.1 \\
            \textit{gradient\_accumulation\_steps}  & 2 & 4 & 2 \\
            \textit{lr\_scheduler\_type}  & \textit{linear} & \textit{linear} & \textit{cosine} \\

        \bottomrule
    \end{tabular}
    \caption{Hiperparámetros de las estrategias avanzadas de optimización}
    \label{tab:ValoresHiperparametrosEstrategias}
\end{table}
...
A nivel teórico, la integración de múltiples modalidades de datos se divide principalmente en dos tipos \cite{ramachandram2017deep}: 
\begin{itemize}
...
En la práctica empírica y en las competiciones de Ciencia de Datos, el \textbf{\textit{late fusion} mediante predicción estática} se ha estandarizado como la metodología más eficiente. Al separar la fase de inferencia de la fase de combinación, esta técnica permite ajustar y recalcular el peso de \textbf{cada modelo} miles de veces en cuestión de milisegundos, sin necesidad de repetir la costosa fase de extracción de características y cómputo matemático en la unidad de procesamiento gráfico (GPU), optimizando así tanto el tiempo de ejecución como el consumo de memoria de vídeo (\textit{VRAM}).
...
Aunque frecuentemente están ligados en la literatura, es fundamental establecer la diferencia teórica entre el Aprendizaje con Desacuerdo (\textit{LeWiDi}) y el Perspectivismo (\textit{Data Perspectivism}) \cite{basile2021we}. Mientras que \textit{LeWiDi} es un marco metodológico enfocado en cómo gestionar la multiplicidad de etiquetas (preservando la varianza matemática sin importar si el desacuerdo proviene de un error del anotador, ambigüedad sintáctica o factores sociales), el perspectivismo se centra en el origen subyacente de dicho desacuerdo. Por tanto, no todo \textit{LeWiDi} implica un análisis demográfico. 

El perspectivismo es una corriente reciente en la inteligencia artificial que sostiene que gran parte del desacuerdo no es aleatorio, sino que las características sociodemográficas de los anotadores (como su género, edad, raza o nivel educativo) condicionan radicalmente sus respuestas. En la detección de sexismo, por ejemplo, la perspectiva de una mujer y la de un hombre frente a un mismo texto pueden diferir por completo debido a sus vivencias. El perspectivismo se apoya en las técnicas algorítmicas de \textit{LeWiDi} para lograr modelar y representar estas visiones sociológicas del mundo. 

Para capturar y representar estas visiones dependiendo de la naturaleza de la arquitectura utilizada, la literatura propone distintas metodologías de modelado: 
\begin{itemize}
    ...
    \item \textbf{Asignación de roles para Modelos Generativos (\textit{LLMs})}: Los grandes modelos de lenguaje han interiorizado múltiples perspectivas durante su preentrenamiento masivo. Para evocar una visión demográfica específica sin necesidad de reentrenar la red, se utiliza la técnica de \textit{Persona Prompting} (ingeniería de instrucciones basada en roles) \cite{deshpande2023toxicity}. Al inyectar directrices explícitas en el \textit{prompt} del sistema (instruyendo al modelo para que ``actúe como un hombre'' o ``actúe como una mujer''), se condiciona la generación de texto para que adopte patrones lingüísticos asociados estadísticamente a ese rol. No obstante, es fundamental matizar que esta técnica simplemente guía las salidas textuales del modelo basándose en las correlaciones de sus datos de entrenamiento, pero no equivale a autenticar ni reproducir un sesgo cognitivo o una perspectiva humana real. 
\end{itemize}
...
Cuando se abandona el paradigma del consenso absoluto y se entrena a los modelos utilizando \textit{Soft Labels}, las métricas tradicionales basadas en coincidencias binarias (como el \textit{Accuracy} o el \textit{F1-score}) dejan de ser completamente representativas. En estos escenarios, el objetivo es medir qué tan bien se alinea la predicción del modelo con la distribución real de los anotadores humanos.

Durante el entrenamiento, esta alineación se optimiza habitualmente utilizando la \textbf{Entropía Cruzada (\textit{Cross-Entropy})} como función de pérdida. Esta métrica penaliza matemáticamente las divergencias entre la probabilidad predicha por el modelo ($\hat{y}$) y la opinión humana real ($y$):
...
Sin embargo, para la evaluación final en el marco de competiciones recientes de análisis de subjetividad (como la tarea \textit{EXIST}), el rendimiento no se mide con la entropía cruzada, sino que se cuantifica mediante la métrica \textbf{\textit{ICM} (\textit{Information Contrast Measure})}, fundamentada metodológicamente por Amigó y Delgado \cite{amigo2022evaluating}. A diferencia de las funciones de pérdida estándar, el \textit{ICM} se basa en la teoría de la información para evaluar la cantidad de información útil que las predicciones del modelo aportan frente a un modelo base o aleatorio.

Para abarcar las distintas naturalezas de salida de los sistemas de clasificación, la métrica se divide en dos variantes:
\begin{itemize}
    \item \textbf{\textit{ICM-Hard}}: Diseñada para evaluar las predicciones categóricas o rígidas (0 o 1). Mide el contraste de información de la etiqueta absoluta elegida por el modelo frente a la distribución real de los anotadores, penalizando de forma severa aquellas respuestas que contradicen frontalmente a la opinión mayoritaria.
    \item \textbf{\textit{ICM-Soft}}: Diseñada para evaluar las predicciones probabilísticas. Si el modelo logra predecir con exactitud la incertidumbre y el grado de polarización (por ejemplo, arrojando un 60\% de probabilidad de sexismo en un mensaje altamente debatido por los humanos), el \textit{ICM-Soft} recompensa esta flexibilidad del modelo, validando que la máquina ha comprendido de manera correcta la ambigüedad inherente al lenguaje natural.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{\LaTeX{}}: Sistema de composición de textos utilizado para la redacción, estructuración y maquetación de esta memoria. Su elección garantiza un formato académico riguroso, así como una gestión impecable de las referencias bibliográficas y la representación de fórmulas matemáticas. 
\end{itemize}
