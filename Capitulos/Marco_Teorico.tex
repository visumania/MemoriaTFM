...
\begin{itemize}
    \item \textbf{Modelos Multilingües}: Se han utilizado \texttt{mmBERT} \cite{marone2025mmbert}, \texttt{xlm-roberta-base} \cite{mehta2023llm} y \texttt{TwHIN-BERT} \cite{zhang2023twhin}, idóneos para captar estructuras semánticas universales y transferir conocimiento entre idiomas.
    \item \textbf{Modelos específicos para Español}: Se han evaluado \texttt{BETO} \cite{garrido2024maria}, \texttt{BERTin} \cite{de2022bertin} y \texttt{RoBERTuito} \cite{perez2022robertuito}, siendo este último optimizado para el lenguaje informal de redes sociales. 
    \item \textbf{Modelos específicos para Inglés}: Se ha experimentado con \texttt{RoBERTa-Base}, \texttt{BERTweet} \cite{nguyen2020bertweet} (preentrenado en mensajes cortos y jerga digital) y \texttt{Electra-base-discriminator} \cite{clark2020electra} (que introduce un paradigma de reemplazo de tokens más eficiente computacionalmente). 
\end{itemize}
